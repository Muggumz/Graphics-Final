...
int main()
{
	...
	while (!App::IsClosing() && !Input::GetKeyDown(GLFW_KEY_ESCAPE))
	{
		...
		ent_ducky.transform.m_rotation.x += -0.1 * deltaTime;
		ent_ducky.transform.m_rotation.y += -0.1 * deltaTime;
		ent_ducky.transform.m_rotation.z += -0.1 * deltaTime;
		ent_ducky.transform.m_pos.x += 0.5 * deltaTime;
		ent_ducky.transform.m_scale.x += 0.0003 * deltaTime;
		ent_ducky.transform.m_scale.y += 0.0003 * deltaTime;
		ent_ducky.transform.m_scale.z += 0.0003 * deltaTime;

		mat_ducky.m_color.x += -0.5 * deltaTime;
		mat_ducky.m_color.y += 0.5 * deltaTime;
		mat_ducky.m_color.z += -0.5 * deltaTime;
